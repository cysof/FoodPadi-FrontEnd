...
import { CheckCircle, Loader2, Pencil, Trash, XCircle } from "lucide-react";
...
const CropOverlayButton = ({ value }: { value: ICrop }) => {
  ...
  return (
    <div className="relative text-center">
      ...
      <OverlayPanel
        ...
        closeOnEscape
        dismissable={true}
        className={` bg-white drop-shadow-md rounded-[10px] .px-2 text-center max-w-[200px] py-3 w-full`}
      >
        <a
          className={`text-black font-inter font-[600] text-[14px] hover:bg-[#DDE4E6] hover:cursor-pointer flex items-center gap-2 .justify-center py-[10px] px-[12px]`}
          onClick={() => {
            dispatch(setShowUpdateCropModal({ id: value.id, show: true }));
          }}
        >
          <Pencil width={16} height={16} /> Update Crop
        </a>
...
export default CropOverlayButton;
