...
const JoinTheWaitList = () => {
  return (
    <div id='waitlist' className={`py-15 px-3 md:px-10 bg-primary text-white`}>
      ...
      <div className={`flex flex-col items-center sm:items-start sm:justify-center gap-8 px-3 md:px-5`}>
        ...
        <Link className={`font-square text-lg border border-gray-300 hover:bg-white/90 hover:text-primary rounded-full py-2 text-center w-max px-5 uppercase`} target='_blank' href={`https://chat.whatsapp.com/CuueYeE9cQnJkgYgSLUDHC`}>Click to Join</Link>
      </div>
      </div>
    </div>
  )
